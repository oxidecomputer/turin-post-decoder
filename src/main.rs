...
fn decode(code: u32) {
    ...
    if code & 0xffff0000 == 0xea010000 {
        ...
        // EA01xyzz: x = UMC channel (0-0xB), y = training phase (0 or 1),
        // zz = PMU progress code
        let x = (lower >> 12) & 0xf;
        let y = (lower >> 8) & 0xf;
        let zz = lower & 0xff;
        let dimm = umc_to_board_dimm(x);
        println!("PMU Code: 0x{code:08x}");
        println!("  UMC channel:    0x{x:x} ({x})");
        println!("  Board DIMM:     {dimm}");
        println!(
            "  Training phase: {y} ({})",
            match y {
                0 => "phase 0",
        ...
        println!("  Progress code:  0x{zz:02x} ({zz})");
        return;
    }

    // Bootloader codes
    if let Some(info) = decode_bootloader(code) {
        println!("Bootloader Code: 0x{code:08x}");
        println!("  Source:  {}", info.source);
        println!("  Status:  {} (0x{:02x})", info.name, info.status);
        println!("  Detail:  {}", info.description);
        return;
    }

    // ABL test point lookup
    if let Some((name, desc)) = look_up_abl(code) {
        println!("ABL Code: 0x{code:08x}");
        println!("  Name:   {name}");
        println!("  Detail: {desc}");
        return;
    }
    ...
    if code & 0xff000000 == 0xea000000 {
        println!("ABL Code: 0x{code:08x}");
        println!("  Unknown ABL test point (no matching entry)");
        return;
    }

    println!("Unknown POST code: 0x{code:08x}");
}
...
fn decode_bootloader(code: u32) -> Option<BootloaderInfo> {
    ...
    Some(BootloaderInfo { source, name, status, description })
}
